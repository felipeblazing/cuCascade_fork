...
void BM_ConvertGpuToHost(benchmark::State& state)
{
  ...
  for (uint64_t t = 0; t < thread_count; ++t) {
    ...
    thread_gpu_reprs.push_back(std::make_unique<gpu_table_representation>(
      std::make_unique<cudf::table>(std::move(table)), *const_cast<memory_space*>(gpu_space), rmm::cuda_stream_view{}));
  }
  ...
  auto warmup_repr  = std::make_unique<gpu_table_representation>(
    std::make_unique<cudf::table>(std::move(warmup_table)), *const_cast<memory_space*>(gpu_space), warmup_stream.view());
  auto warmup_result =
...
void BM_ConvertHostToGpu(benchmark::State& state)
{
  ...
  for (uint64_t t = 0; t < thread_count; ++t) {
    ...
    auto gpu_repr_temp = std::make_unique<gpu_table_representation>(
      std::make_unique<cudf::table>(std::move(table)), *const_cast<memory_space*>(gpu_space), setup_stream.view());
    auto host_repr =
